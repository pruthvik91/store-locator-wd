	...
	if ((!empty($_POST) && $found)   && ((isset($_POST["btnSubmit"]) && $_POST["btnSubmit"] == "Submit"))) {
		foreach ($rows1 as $rs1) {
			if ($rs1['barcode_no'] != null && !empty($rs1['barcode_no'])) {
				...
				$line_m = $line_mm  = isset($_POST['line_m'])&&!empty($_POST['line_m']) ? intval($_POST['line_m']) : 0;
				$line_l = $line_ll  = isset($_POST['line_l'])&&!empty($_POST['line_l']) ? intval($_POST['line_l']) : 0;
				$line_xl = $line_xll  = isset($_POST['line_xl'])&&!empty($_POST['line_xl']) ? intval($_POST['line_xl']) : 0;
				$line_xxl = $line_xxll  = isset($_POST['line_xxl'])&&!empty($_POST['line_xxl']) ? intval($_POST['line_xxl']) : 0;
